...
if($_SESSION['loggedin']==true){
    ...
    if($userKont['loginID'] == $_SESSION['loginid']){
        $pageTitle = "Hakkımızda Tab Yönetimi";
        $mpage = "content";
        $maltpage = "abouttab";
        $process = $_GET['process'];
        $processTitle = "Hakkımızda Tab Yönetimi";

        $db->where ("id", $_GET["id"]);
        $page = $db->getOne ("aboutus");

        if($process == "edit"){
            $db->where ("id", $_GET["tabID"]);
            $editP = $db->getOne ("aboutus_tab");
        }
...
<body class="kt-offcanvas-panel--right kt-header--fixed kt-header-mobile--fixed kt-aside--enabled kt-aside--fixed kt-page--loading">
                                        ...
                                        <div class="kt-portlet__head-toolbar">
                                            <div class="kt-portlet__head-wrapper">
                                                <div class="kt-portlet__head-actions">
                                                    <a href="?process=add&id=<?php echo $_GET['id'] ?>" class="btn btn-brand btn-elevate btn-icon-sm"><i class="fa fa-plus"></i> Yeni Kayıt</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="kt-portlet__head">
                                            ...
                                            <div class="kt-portlet__head-toolbar">
                                                <div class="kt-portlet__head-wrapper">
                                                    <div class="kt-portlet__head-actions">
                                                        <?php if($process == "edit"){?>
                                                            <button type="button" class="btn btn-success" data-id="<?php echo $editP['id'];?>" data-type="tabEdit" name="set"><i class="fa fa-edit"></i> Düzenle</button>
                                                        <?php }else{?>
                                                            <button type="button" class="btn btn-success"  data-type="tabAdd" name="set" ><i class="fa fa-plus"></i> Ekle</button>
                                                        <?php }?>
                                                    </div>
                                                </div>
                                            </div>
    ...
    <script>
        ...
        function table(id){
            var data = [];
            data.push({name : "type",value : "tabList"},{name : "aboutID",value : ""+id+""});
            $.ajax({
        ...
        function pageset(id,type){
            ...
            data.push({name : "type",value : type},{name : "aboutID",value : "<?php echo $page['id'];?>"});
            if(type=="tabEdit" || type=="tabDel"){
                data.push({name : "tabID",value : id});
            }
            $.ajax({
